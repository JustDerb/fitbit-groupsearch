...
function search_keywordMatch($search)
{
	...
	// Exact match
	$query .= "select id, name, description, members, url, ".($weight+4)." weight \n";
	$query .= "from groups \n";
	$query .= "    where name='".$search."' \n";
	$query .= "union \n";
	$query .= "select id, name, description, members, url, ".($weight+3)." weight \n";
	$query .= "from groups \n";
	$query .= "    where description='".$search."' \n";
	$query .= "union \n";
	//Partial search
	$query .= "select id, name, description, members, url, ".($weight+2)." weight \n";
	$query .= "from groups \n";
	$query .= "    where name like '%".$search."%' \n";
	$query .= "union \n";
	$query .= "select id, name, description, members, url, ".($weight+1)." weight \n";
	$query .= "from groups \n";
	...
	foreach($searches as $keyword)
	{
		...
		$query .= "select id, name, description, members, url, ".($weight)." weight \n";
		$query .= "from groups \n";
